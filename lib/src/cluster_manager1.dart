...
class ClusterManager1<T extends ClusterItem> {
  ClusterManager1(this._items, this.updateMarkers,
      {Future<Marker> Function(Cluster1<T>)? markerBuilder,
  ...
  final Future<Marker> Function(Cluster1<T>) markerBuilder;
  ...
  void _updateClusters() async {
    List<Cluster1<T>> mapMarkers = await getMarkers();
  ...
  Future<List<Cluster1<T>>> getMarkers() async {
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!)
      return visibleItems.map((i) => Cluster1<T>.fromItems([i])).toList();

    List<Cluster1<T>> markers;
  ...
  List<Cluster1<T>> _computeClustersWithMaxDist(
  ...
  List<Cluster1<T>> _computeClusters(
      List<T> inputItems, List<Cluster1<T>> markerItems,
      {int level = 5}) {
    ...
    markerItems.add(Cluster1<T>.fromItems(items));
  ...
  static Future<Marker> Function(Cluster1) get _basicMarkerBuilder =>
      (cluster) async {
        return Marker(
